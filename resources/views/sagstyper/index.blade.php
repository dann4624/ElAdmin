...
@section('content')
    <form action="{{request()->segment(count(request()->segments()))}}" method="post" id="create_form">
        ...
        <input type="number" placeholder="Pris" name="pris" id="create_pris" step="0.01">
            ...
            @foreach($data as $object)
                <tr>
                    <form action="{{request()->segment(count(request()->segments()))}}/{{$object['id']}}" method="post" id="edit_form">
                        ...
                        <td>
                            <input type="number" value="{{$object['pris']}}" placeholder="Pris" name="pris" id="edit_pris_{{$object['id']}}" step="0.01" readonly>
                        </td>
                    <td id="cms">
