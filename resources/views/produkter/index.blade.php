...
@section('content')
    <form action="{{request()->segment(count(request()->segments()))}}" method="post" id="create_form">
        ...
        <select name="model_id" id="create_model_id">
            @foreach($modeller as $model)
                <option value="{{$model['id']}}">{{$model['navn']}}</option>
            @endforeach
        </select>
            ...
            @foreach($data as $object)
                <tr>
                    <form action="{{request()->segment(count(request()->segments()))}}/{{$object['id']}}" method="post" id="edit_form">
                        ...
                        <td>
                            <select name="model_id" id="edit_model_id_{{$object['id']}}" disabled>
                                @foreach($modeller as $model)
                                    <option value="{{$model['id']}}" @if($object['model']['id'] == $model['id']) selected @endif>{{$model['fabrikant']['navn']}} {{$model['navn']}}</option>
                                @endforeach
                            </select>
                        </td>
                        <td id="cms">
